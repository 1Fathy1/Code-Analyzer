<?php
// 1. تفعيل الـ CORS والـ Headers عشان الـ Node.js يقدر يكلم السيرفر ده بدون مشاكل حماية
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
header("Access-Control-Allow-Methods: POST");
header("Access-Control-Allow-Headers: Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With");

// لو الطلب نوعه OPTIONS (تأكيد الأمان قبل الـ POST) بنقفل الطلب بنجاح
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

// 2. التأكد أن الطلب المرسل هو POST
...
// 3. استدعاء مكتبة الـ PHP-Parser (تأكدي من وجود فولدر vendor أو الـ autoload في مشروعك)
// لو الـ vendor في مكان تاني تقدري تعدلي المسار بتاعه هنا
...
// 4. كلاس الـ SecurityTaintVisitor الخاص بكِ (مع تعديل الـ Key لـ type)
class SecurityTaintVisitor extends NodeVisitorAbstract {
    ...
    private function addFinding($type, $line, $severity) {
        // التعديل الأول 🛠️: تغيير الـ Key لـ "type" عشان يتطابق مع السي شارب والـ React
        $this->findings[] = ['line' => $line, 'severity' => $severity, 'type' => $type];
    }
}

// 5. كلاس الـ AwareXPHPAnalyzer الخاص بكِ
class AwareXPHPAnalyzer {
    private $parser;
    public function __construct() { 
        $this->parser = (new ParserFactory())->createForNewerVersion(); 
    }

    public function analyze($userCode, $userMode) {
        try {
            $ast = $this->parser->parse($userCode);
            if ($ast === null) return ['status' => 'success', 'vulnerabilities' => []];
            ...
            // التعديل الثاني 🛠️: إرجاع الـ "vulnerabilities" مباشرة بشكلها الجديد والموحد
            return [
                'status' => 'success',
...
// 6. استلام البيانات المرسلة من الـ Node.js وتشغيل الفحص الويب
$inputData = json_decode(file_get_contents("php://input"), true);
$userCode = isset($inputData['code']) ? $inputData['code'] : null;
$userMode = isset($inputData['mode']) ? $inputData['mode'] : null;

if (empty($userCode) || empty($userMode)) {
    http_response_code(400);
    echo json_encode(["status" => "error", "message" => "المعاملات المرسلة (code أو mode) ناقصة!"]);
    exit();
}

// تشغيل الـ Analyzer وإرسال الـ JSON الموحد
$analyzer = new AwareXPHPAnalyzer();
$responseResult = $analyzer->analyze($userCode, $userMode);
